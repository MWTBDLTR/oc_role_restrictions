...
// @version      0.9.3
// @description  Highlight role restrictions and best roles in OC 2.0. Incorporates API weight calculations. Pair with OC Success Chance 2.0.
...
(function () {
  ...
  function getLowerFromWeight(weight, roleCount = 3) {
    ...
    if (weight < 15) {
      ...
      val = 62 + (weight - 15) * 0.6;
    } else {
      val = 74 + (weight - 35) * 0.2;
    }
  ...
  function classifyOcRoleInfluence(ocName, roleName, roleCount = 3) {
    ...
    // Fallback to static/faction defaults
    const ocInfo = ocRoleInfluence[ocName];
    const roleData = ocInfo?.find((r) => r.role === roleName);
    const lower = roleData ? roleData.lower : 70;
    
    // Handle the "next highest lower bound" logic for static data
    let upper = lower + 10;
    if (ocInfo) {
      const roleLowers = ocInfo.map((role) => role.lower).sort();
  ...
  function processCrime(wrapper) {
    ...
    // Validate if this scenario is supported by API
    ...
    roleEls.forEach((roleEl) => {
      ...
      if (slotHeader) {
        if (chance >= evaluation.upper) {
          // Optional: slotHeader.style.backgroundColor = "#ca6f1e";
    ...
    if (DEBUG) console.log(`[OCRoleRestrictions] Processed: ${crimeTitle} with ${totalRolesInCrime} roles.`);
  }

  function refreshCrimes() {
    const allCrimes = document.querySelectorAll('[class*="wrapper__"][data-oc-id]');
  ...
  function fetchApiData() {
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url: endpoints.weights,
      onload: (res) => { if (res.status === 200) { ocWeights = JSON.parse(res.responseText); refreshCrimes(); } }
    });
    GM_xmlhttpRequest({
      method: "GET",
      url: endpoints.scenarios,
      onload: (res) => { if (res.status === 200) { supportedScenarios = JSON.parse(res.responseText); refreshCrimes(); } }
    });
    GM_xmlhttpRequest({
      method: "GET",
      url: endpoints.names,
      onload: (res) => { if (res.status === 200) { apiRoleNames = JSON.parse(res.responseText); refreshCrimes(); } }
    });
  }
  ...
  function getFactionId() {
    ...
    try {
      ...
      if (!ft || !ft.href) return null;
      ft.href.split("#")[1].split("&").forEach((elem) => {
        if (elem[0] == "a") factionId = elem.split("=")[1];
      });
    } catch (e) {
      ...
      onload: async function (response) {
        if (response.status == 200) {
          try {
            ...
            console.error("[OCRoleRestrictions] Failed to parse faction restrictions:", error.message);
          }
        }
        return cb();
      },
  ...
  function setupMutationObserver(root) {
    const observer = new MutationObserver(() => {
      const tabTitle = document.querySelector('button[class*="active__"] span[class*="tabName__"]')?.textContent.trim();
  ...
  if (factionId) {
    ...
    console.log("[OCRoleRestrictions] No faction id, using defaults.");
    cb();
  }

  // Dependency: waitForKeyElements
  function waitForKeyElements(selectorOrFunction, callback, waitOnce = true, interval = 300, maxIntervals = -1) {
    ...
    if (targetsFound) {
      targetNodes.forEach(function (targetNode) {
        ...
        if (!targetNode.getAttribute(attrAlreadyFound)) {
          var cancelFound = callback(targetNode);
          if (!cancelFound) targetNode.setAttribute(attrAlreadyFound, true);
        }
      });
    }
