...
(function () {
  ...
  function classifyOcRoleInfluence(ocName, roleName, roleCount = 3) {
    ...
    return {
      lower,
      upper: lower + 10,
      source: roleData ? "Default Value" : "Fallback Value",
    };
  }

  function processCrime(wrapper) {
    // Use data-oc-id attribute for reliable wrapper identification (robust against hash changes)
    const ocId = wrapper.getAttribute("data-oc-id");
    if (!ocId || crimeData[ocId]) return;

    // Wildcard selector for panel title - matches any class containing 'panelTitle' followed by underscores and alphanumeric chars
    const titleEl = wrapper.querySelector('p[class*="panelTitle___"]');
    if (!titleEl) return;
    ...
    // Wildcard selector for role titles - matches any class containing 'title' followed by underscores and alphanumeric chars
    const roles = [];
    const roleEls = wrapper.querySelectorAll('.class*="title___"]');
    ...
    roleEls.forEach((roleEl) => {
      ...
      // Next sibling element for success chance
      ...
      // Wildcard selector for slot header button - matches any class containing 'slotHeader' followed by underscores and alphanumeric chars
      const slotHeader = roleEl.closest('button[class*="slotHeader___"]');
      if (slotHeader) {
    ...
    if (DEBUG) {
      console.log(
        `[OCRoleRestrictions] Processed: ${crimeTitle} using ${totalRolesInCrime} roles.`,
    )};
  }

  function refreshCrimes() {
    // Use data-oc-id attribute for reliable wrapper identification (robust against hash changes)
    const allCrimes = document.querySelectorAll(".wrapper___U2Ap7[data-oc-id]");
  ...
  function getFactionId() {
    ...
    try {
      ...
      console.log("Faction ID search failed.");
    }
    return factionId;
  }
  ...
  function setupMutationObserver(root) {
    const observer = new MutationObserver(() => {
      // Wildcard selector for tab elements - matches any class containing 'active' followed by underscores and alphanumeric chars
      const tabTitle = document
        .querySelector('button[class*="active___"] span[class*="tabName___"]')
        ?.textContent.trim();
    ...
    // Observe root element for child changes
  ...
  // Find the new root container using a more resilient approach
  function findCrimesRoot() {
    // Try multiple approaches to locate the crimes page root
    let root = document.querySelector("#faction-crimes"); // New ID from Torn update

    if (!root) {
      // Fallback: look for any element with data-oc-id (indicates we're on a crimes page)
      const crimeWrapper = document.querySelector("[data-oc-id]");
      if (crimeWrapper) {
        return crimeWrapper;
      }

      // Another fallback: try to find the tab container
      root = document.querySelector(
        "#faction-crimes-root, #faction-tab-container, .crimes-wrapper",
      );
    }
  ...
  const startCallback = () => {
    waitForKeyElements(findCrimesRoot(), (root) => setupMutationObserver(root));
  };
